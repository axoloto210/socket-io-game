...
describe('Item Effects', () => {
  ...
  test('should detect when mukouEffect applies', () => {
    const mukou: Item = { itemId: 2, itemName: 'ムコウカ', itemEffect: '相手のアイテム効果を無効にする' };
    const risky: Item = { itemId: 3, itemName: 'リスキー', itemEffect: '勝った場合、相手に追加で1ダメージ' };
  ...
  test('should detect abekobe effect correctly', () => {
    const abekobe: Item = { itemId: 4, itemName: 'アベコベ', itemEffect: '数字の大小が逆転する' };
    const risky: Item = { itemId: 3, itemName: 'リスキー', itemEffect: '勝った場合、相手に追加で1ダメージ' };
  ...
  test('should get correct base point for draw situations', () => {
    ...
    const normalResult = items.getDrawBasePoint(basePoint, undefined, undefined);
